class CFTDiscordObject_Video : CFTDiscordObject
{
	/*
	...
	*/
	...
	void CFTDiscordObject_Video(string url, int width = -1, int height = -1)
	{
		m_width = width;
		m_height = height;
	...
	private string GetWidthField()
	{
		if (m_width > -1)
		{
			return CFTDiscordHelper.GetJSONLineOthers("width", m_width.ToString());
		}
		return "";
	}
	...
	private string GetHeightField()
	{
		if (m_height > -1)
		{
			return CFTDiscordHelper.GetJSONLineOthers("height", m_height.ToString());
		}
		return "";
	}
